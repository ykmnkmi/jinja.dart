...
void main() {
  group('Syntax', () {
    test('call', () {
      ...
      expect(tmpl.renderMap(), equals('abdfh'));
    });

    test('slicing', () {
      final tmpl = env.fromString('{{ [1, 2, 3][:] }}|{{ [1, 2, 3][::-1] }}');
      expect(tmpl.renderMap(), equals('[1, 2, 3]|[3, 2, 1]'));
    });

    test('attr', () {
      final foo = {'bar': 42};
      final tmpl = env.fromString('{{ foo.bar }}|{{ foo["bar"] }}');
      expect(tmpl.renderMap({'foo': foo}), equals('42|42'));
    });

    test('subscript', () {
      final foo = [0, 1, 2];
      final tmpl = env.fromString('{{ foo[0] }}|{{ foo[-1] }}');
      expect(tmpl.renderMap({'foo': foo}), equals('0|2'));
    });

    test('tuple', () {
      // tuple is list
      final tmpl = env.fromString('{{ () }}|{{ (1,) }}|{{ (1, 2) }}');
      expect(tmpl.renderMap(), equals('[]|[1]|[1, 2]'));
    });

    test('math', () {
      var tmpl = env.fromString('{{ (1 + 1 * 2) - 3 / 2 }}|{{ 2**3 }}');
      expect(tmpl.renderMap(), equals('1.5|8'));
    });

    test('div', () {
      var tmpl = env.fromString('{{ 3 // 2 }}|{{ 3 / 2 }}|{{ 3 % 2 }}');
      expect(tmpl.renderMap(), equals('1|1.5|1'));
    });

    test('unary', () {
      var tmpl = env.fromString('{{ +3 }}|{{ -3 }}');
      expect(tmpl.renderMap(), equals('3|-3'));
    });

    test('concat', () {
      var tmpl = env.fromString('{{ [1, 2] ~ "foo" }}');
      expect(tmpl.renderMap(), equals('[1, 2]foo'));
    });

    test('compare', () {
      var tmpl = env.fromString('{{ 1 > 0 }}');
      expect(tmpl.renderMap(), equals('true'));
    ...
    test('compare parens', () {
      ...
      expect(tmpl.renderMap({'i': 2, 'j': 3}), equals('false'));
    });

    test('compare compound', () {
      final data = {'a': 4, 'b': 2, 'c': 3};
      var tmpl = env.fromString('{{ 4 < 2 < 3 }}');
      expect(tmpl.renderMap(data), equals('false'));
      ...
      expect(tmpl.renderMap(data), equals('false'));
      tmpl = env.fromString('{{ 4 > 2 < 3 }}');
      expect(tmpl.renderMap(data), equals('true'));
      tmpl = env.fromString('{{ a > b < c }}');
      expect(tmpl.renderMap(data), equals('true'));
    });
    ...
    test('inop', () {
      ...
      expect(tmpl.renderMap(), equals('true|false'));
    });
    ...
    test('filter priority', () {
      ...
      expect(tmpl.renderMap(), equals('FOOBAR'));
    });
    ...
    test('triling comma', () {
      ...
      expect(tmpl.renderMap(), equals('[1, 2]|[1, 2]|{1: 2}'));
    });
    ...
    test('string concatenation', () {
      ...
      expect(tmpl.renderMap(), equals('foobarbaz'));
    });

    test('notin', () {
      final tmpl = env.fromString('{{ not 42 in bar }}');
      expect(tmpl.renderMap({'bar': range(100)}), equals('false'));
    });

    test('operator precedence', () {
      final tmpl = env.fromString('{{ 2 * 3 + 4 % 2 + 1 - 2 }}');
      expect(tmpl.renderMap(), equals('5'));
    });

    test('implicit subscribed tuple', () {
      // tuple is list
      final tmpl = env.fromString('{{ foo[1, 2] }}');
      expect(tmpl.renderMap({'foo': Foo()}), equals('[1, 2]'));
    });
    ...
    test('raw2', () {
      ...
      expect(tmpl.renderMap(), equals('{{ FOO }} and {% BAR %}'));
    });
    ...
    test('const', () {
      final tmpl = env
          .fromString('{{ true }}|{{ false }}|{{ none }}|{{ none is defined }}|'
              '{{ missing is defined }}');
      expect(tmpl.renderMap(), equals('true|false|null|true|false'));
    });
    ...
    test('parse unary', () {
      ...
      expect(tmpl.renderMap({'foo': foo}), equals('-42'));
