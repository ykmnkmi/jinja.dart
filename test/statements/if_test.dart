...
void main() {
  group('if', () {
    ...
    test('else', () {
      final Template template = env.fromString('{% if false %}XXX{% else %}...{% endif %}');
      expect(template.renderMap(), equals('...'));
    });

    test('empty', () {
      final Template template = env.fromString('[{% if true %}{% else %}{% endif %}]');
      expect(template.renderMap(), equals('[]'));
    });

    test('complete', () {
      final Template template = env.fromString('{% if a %}A{% elif b %}B{% elif c == d %}'
          'C{% else %}D{% endif %}');
