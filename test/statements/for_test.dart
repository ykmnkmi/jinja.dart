...
void main() {
  group('for', () {
    ...
    test('simple', () {
      Template template = env.fromString('{% for item in seq %}{{ item }}{% endfor %}');
      expect(template.renderWr(seq: range(10)), equals('0123456789'));
    });

    test('else', () {
      Template template = env.fromString('{% for item in seq %}XXX{% else %}...{% endfor %}');
      expect(template.render(), equals('...'));
    });

    test('else scoping item', () {
      Template template = env.fromString('{% for item in [] %}{% else %}{{ item }}{% endfor %}');
      expect(template.renderWr(item: 42), equals('42'));
    });

    test('empty blocks', () {
      Template template = env.fromString('<{% for item in seq %}{% else %}{% endfor %}>');
      expect(template.render(), equals('<>'));
    });
    ...
    test('context vars', () {
      ...
      for (Iterable<int> seq in <Iterable<int>>[slist, slist.reversed]) {
        ...
        List<String> parts = template.render(<String, Object>{'seq': seq}).split('###');
    ...
    test('cycling', () {
      ...
      expect(template.renderWr(seq: range(4), through: <String>['<1>', '<2>']), equals('<1><2>' * 4));
    });
    ...
    test('lookaround', () {
      Template template = env.fromString('''{% for item in seq -%}
        ...
        {%- endfor %}''');
      expect(template.renderWr(seq: range(4)), equals('x-0-1|0-1-2|1-2-3|2-3-x|'));
    });
    ...
    test('changed', () {
      Template template = env.fromString('''{% for item in seq -%}
            {{ loop.changed(item) }},
        {%- endfor %}''');
      expect(template.renderWr(seq: <int>[null, null, 1, 2, 2, 3, 4, 4, 4]),
          equals('true,false,true,true,false,true,true,false,false,'));
    });

    test('scope', () {
      Template template = env.fromString('{% for item in seq %}{% endfor %}{{ item }}');
      expect(template.renderWr(seq: range(10)), equals(''));
    });

    test('varlen', () {
      Template template = env.fromString('{% for item in iter %}{{ item }}{% endfor %}');
    ...
    test('noniter', () {
      Template template = env.fromString('{% for item in none %}...{% endfor %}');
    ...
    test('looploop', () {
      Template template = env.fromString('''{% for row in table %}
            ...
            {%- endfor %}
        {%- endfor %}''');
      expect(template.renderWr(table: <String>['ab', 'cd']), '[1|1][1|2][2|1][2|2]');
    });
    ...
    test('reversed bug', () {
      Template template = env.fromString('{% for i in items %}{{ i }}'
          '{% if not loop.last %}'
          ',{% endif %}{% endfor %}');
      expect(template.renderWr(items: <int>[3, 2, 1].reversed), '1,2,3');
    });
    ...
    test('scoped special var', () {
      Template template = env.fromString('{% for s in seq %}[{{ loop.first }}{% for c in s %}'
          '|{{ loop.first }}{% endfor %}]{% endfor %}');
      expect(template.renderWr(seq: <String>['ab', 'cd']), '[true|true|false][false|true|false]');
    });
