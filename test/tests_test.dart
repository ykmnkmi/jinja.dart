...
void main() {
  group('Test', () {
    test('defined', () {
      ...
      expect(tmpl.renderMap(), equals('false|true'));
    });
    ...
    test('types', () {
      ...
      expect(tmpl.renderMap({'md': MyMap()}), equals('true'));
    ...
    test('same as', () {
      ...
      expect(tmpl.renderMap({'foo': false}), equals('true|false'));
    });

    test('no paren for arg 1', () {
      final tmpl = env.fromString('{{ foo is sameas none }}');
      expect(tmpl.renderMap({'foo': null}), equals('true'));
    });

    test('escaped', () {
      final tmpl = env.fromString('{{  x is escaped }}|{{ y is escaped  }}');
      expect(
          tmpl.renderMap({'x': 'foo', 'y': Markup('foo')}), equals('false|true'));
    });
